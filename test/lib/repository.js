...
describe('Repository', function () {
  ...
  describe('setup()', function () {
    it(`sets remote repositories and user's info`, function () {
      ...
      repo.setup()
      assert(shell.exec(`git remote show -n ${repo.remote.upstream.name} | grep '^*' | cut -b 10-`).stdout.replace('\n', '') === repo.remote.upstream.name)
      assert(shell.exec(`git remote show -n ${repo.remote.head.name} | grep '^*' | cut -b 10-`).stdout.replace('\n', '') === repo.remote.head.name)
      assert(shell.exec('git config user.name').stdout.replace('\n', '') === repo.user.name)
      assert(shell.exec('git config user.email').stdout.replace('\n', '') === repo.user.email)
    })
  })
  ...
  describe('hasConflicts()', function () {
    it('returns true/false whether cherry-pick/merge succeed', function () {
      ...
      assert.ifError(repo.hasConflicts('cherry-pick', latestHash, '--no-commit'))
      assert.ifError(repo.hasConflicts('merge', repo.remote.origin.defaultBranch, '--no-commit'))
    })
  })
})
